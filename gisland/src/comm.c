    ...
    *prompt = '\0';
    ...
    if (PRF_FLAGGED(d->character, PRF_DISPAUTO) && len < sizeof(prompt)) {
      ...
      if (GET_MANA(ch) << 2 < GET_MAX_MANA(ch) && len < sizeof(prompt)) {
        count = snprintf(prompt + len, sizeof(prompt) - len, "<%d/%dNen> ", GET_MANA(ch), GET_MAX_MANA(ch));
        if (count >= 0)
          len += count;
      }
	  ...
	  /*if (GET_POS(d->character) == POS_FIGHTING) && len < sizeof(prompt)) {
        count = snprintf(prompt + len, sizeof(prompt) - len, "vs. <%d> ", ((GET_HIT(vict) * 100) / GET_MAX_HIT(vict)));
        if (count >= 0)
          len += count;
      }*/
    }
