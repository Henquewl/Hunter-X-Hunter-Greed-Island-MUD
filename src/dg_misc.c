...
void do_dg_cast(void *go, struct script_data *sc, trig_data *trig, int type, char *cmd)
{
  ...
  if (!caster) {
    ...
    /* set the caster's name to that of the object, or the Game Masters.... */
    if (type==OBJ_TRIGGER)
      caster->player.short_descr =
        strdup(((struct obj_data *)go)->short_description);
    else if (type==WLD_TRIGGER)
      caster->player.short_descr = strdup("The Game Masters");
 ...
 * - allow_Game Masters is false when called by %force%, for instance, while true for
 * %teleport%. - Welcor */
