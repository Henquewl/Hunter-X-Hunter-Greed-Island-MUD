...
{
  if (obj) {
    *cost += MAX(0, (GET_OBJ_RENT(obj) - GET_OBJ_RENT(obj)));
    ...
    obj_data *obj, long *cost, long *nitems, int display, int factor)
{
  ...
  if (obj) {
    if (!Crash_is_unrentable(obj)) {
      (*nitems)++;
      *cost += MAX(0, ((GET_OBJ_RENT(obj) - GET_OBJ_RENT(obj)) * factor));
      if (display) {
        sprintf(buf, "$n tells you, '%5d coins for %s..'",
                ((GET_OBJ_RENT(obj) - GET_OBJ_RENT(obj)) * factor), OBJS(obj, ch));
        act(buf, FALSE, recep, 0, ch, TO_VICT);
      }
    }
    ...
    char *arg, int mode)
{
  ...
  if (!CMD_IS("offer") && !CMD_IS("rent") && !CMD_IS("leave"))
    return (FALSE);
...
/*  if (cost > GET_GOLD(ch) + GET_BANK_GOLD(ch)) {
    ...
    if (mode == RENT_FACTOR) {
      act("$n stores your belongings and teleport you out of the island.", FALSE, recep, 0, ch, TO_VICT);
      ...
      act("$n stores your belongings and teleport you out of the island.\r\n"
    ...
    act("$n teleported $N, $S left the island.", FALSE, recep, 0, ch, TO_NOTVICT);
