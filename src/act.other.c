...
ACMD(do_practice)
{
  ...
  if (*arg)
    send_to_char(ch, "You can only practice skills with a nen master.\r\n");
  else
    list_skills(ch);
}
...
void show_happyhour(struct char_data *ch)
{
  ...
  if ((IS_HAPPYHOUR) || (GET_LEVEL(ch) >= LVL_GRGOD))
  {
      ...
      sprintf(happyqp,   "%s+%d%%%s to Booster Pack drop per kill\r\n", CCYEL(ch, C_NRM), HAPPY_QP,   CCNRM(ch, C_NRM));
      ...
      send_to_char(ch, "Greed Island Happy Hour!\r\n"
                       "------------------\r\n"
