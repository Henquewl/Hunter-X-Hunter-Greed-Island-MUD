...
#define PERS(ch, vict)   (CAN_SEE(vict, ch) ? GET_NAME(ch) : (GET_LEVEL(ch) > LVL_IMMORT ? "something" : "someone"))
...
/** 1 if ch is manipulator class, 0 if not. */
#define IS_MANIPULATOR(ch)		(!IS_NPC(ch) && \
        (GET_CLASS(ch) == CLASS_MANIPULATOR))

/** 1 if ch is specialist class, 0 if not. */		
#define IS_SPECIALIST(ch)		(!IS_NPC(ch) && \
        (GET_CLASS(ch) == CLASS_SPECIALIST))
